<?php
if (isset($_FILES['fotopic2'])) {
    $fotopic2 = $_FILES['fotopic2'];
    $extension = strtolower(pathinfo($fotopic2['name'], PATHINFO_EXTENSION));
	  $time = time();
    //$nombre = "{$_POST['nombre_fotopic2']}_$time.$extension";
    $nombre = "{$_POST['nombre_fotopic2']}$time.$extension";
		$sizefotopic2 = $_FILES['fotopic2']['size'];

if (($extension=='jpeg' || $extension=='jpe' || $extension=='jpg' || $extension=='png' || $extension=='gif' ) && ($sizefotopic2<=10000000000))
{


if (move_uploaded_file($fotopic2['tmp_name'], "pictures/$nombre")) {
