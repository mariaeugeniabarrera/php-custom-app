<?php
if (isset($_FILES['fotopic4'])) {
    $fotopic4 = $_FILES['fotopic4'];
    $extension = strtolower(pathinfo($fotopic4['name'], PATHINFO_EXTENSION));
	  $time = time();
    //$nombre = "{$_POST['nombre_fotopic4']}_$time.$extension";
    $nombre = "{$_POST['nombre_fotopic4']}$time.$extension";
		$sizefotopic4 = $_FILES['fotopic4']['size'];

if (($extension=='jpeg' || $extension=='jpe' || $extension=='jpg' || $extension=='png' || $extension=='gif' ) && ($sizefotopic4<=10000000000))
{


if (move_uploaded_file($fotopic4['tmp_name'], "pictures/$nombre")) {
