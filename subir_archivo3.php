<?php
if (isset($_FILES['fotopic3'])) {
    $fotopic3 = $_FILES['fotopic3'];
    $extension = strtolower(pathinfo($fotopic3['name'], PATHINFO_EXTENSION));
	  $time = time();
    //$nombre = "{$_POST['nombre_fotopic3']}_$time.$extension";
    $nombre = "{$_POST['nombre_fotopic3']}$time.$extension";
		$sizefotopic3 = $_FILES['fotopic3']['size'];

if (($extension=='jpeg' || $extension=='jpe' || $extension=='jpg' || $extension=='png' || $extension=='gif' ) && ($sizefotopic3<=10000000000))
{


if (move_uploaded_file($fotopic3['tmp_name'], "pictures/$nombre")) {
